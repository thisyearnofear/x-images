...
const GalleryPage = () => {
  ...
  const handleImageLoad = async (imgElement, imageId) => {
    if (!imageId) return
  ...
  const handleImageClick = (e, image) => {
    ...
    const clone = clickedItem.querySelector('img').cloneNode(true)
    ...
    // Add safety check for dimensions and provide default ratio
    const defaultRatio = 16/9 // fallback to standard aspect ratio
    const imgRatio = image.dimensions?.width && image.dimensions?.height
    ...
    originalImg.onload = () => {
      if (previewRef.current) {
        previewRef.current.querySelector('.preview-image').appendChild(originalImg)
        requestAnimationFrame(() => {
  ...
  const closePreview = () => {
    ...
    const gridItem = document.querySelector(`[data-image-id="${selectedImage.id}"]`)
    ...
    cloneRef.current.addEventListener('transitionend', handleTransitionEnd, { once: true })
  }
        ...
        <div className="content">
          ...
          {isPreviewOpen && (
            <div 
              ...
              ref={previewRef}
              onClick={(e) => {
                // Close preview when clicking the background
                ...
                <div className="preview-details">
                  <div className="preview-header">
                    ...
                    <div className="preview-meta">
                      <p className="attribution">Captured by {selectedImage.attribution}</p>
                      <p className="timestamp">
                        Added on {new Date(selectedImage.timestamp).toLocaleDateString()} at{' '}
                  ...
                  <div className="preview-stats">
                    ...
                    <div className="stat">
                      ...
                      <span className="stat-value">
                        {Math.round((selectedImage.dimensions?.width * selectedImage.dimensions?.height) / 1000000 * 100) / 100} MP
                      </span>
                    </div>
                  </div>
                  <a 
